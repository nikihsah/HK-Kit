...
9. Candidate Registry может содержать только конкретные объекты HK-RDB
   ...
   Узкое исключение — локальные экземпляры Умений по skills.overview:
   они используют character-skill.* как локальный ID и не выдают его за ID HK-RDB.
...
11. MODE CREATE должен завершиться полноценным рассчитанным чарником,
    ...
    После успешного аудита ИИ предложит: создать изображение, заполнить
    официальный лист, изменить персонажа или завершить работу. Изображение
    и лист создаются только после явного выбора игрока.
